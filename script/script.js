...
function changefunc() {
    if (change_2.classList.contains("change")) {
        ...
        for (let i = 0; i < change2Btn.length; i++) {
            ...
            else if (change2Btn[i].innerHTML == "∛x")
                change2Btn[i].innerHTML = "√x";
    ...
    for (let i = 0; i < change2Btn.length; i++) {
        if (change2Btn[i].innerHTML == "x<sup>2</sup>")
            ...
            change2Btn[i].innerHTML = "e<sup>x</sup>";
    }
}
...
function degree_dms(n) {
    let degree = 0, min = 0, sec = 0;
    degree = Math.trunc(n);
    min = Math.trunc((Math.abs(n) - Math.floor(n)) * 60);
    sec = Math.trunc((Math.abs((Math.abs(n) - Math.floor(n)) * 60) -
