...
let trigbtn = document.querySelectorAll(".trignoBtn");
...
for (let i = 0; i < btns.length; i++) {
  btns[i].addEventListener("click", () => {
    switch (btns[i].innerHTML) {
      ...
      case "n!": {
        operArr.push("!");
        let n = exArr[exArr.length - 1];
        exArr.pop();
        exArr.push(factorial(n));
        break;
      }
    }
    display(operArr);
...
fixToExpBtn.addEventListener("click", () => {
  if (fixToExpBtn.innerHTML == "F-E") {
    ...
    document.getElementById("fixExp").style.backgroundColor = "#91c1e7";
    fixedToExp = true;
  } else {
    fixToExpBtn.innerHTML = "F-E";
    document.getElementById("fixExp").style.backgroundColor = "transparent";
    fixedToExp = false;
  }
});
...
function changefunc() {
  if (change_2.classList.contains("change")) {
    ...
    document.getElementById("change_2").style.backgroundColor = "#f7f7f8";
  ...
  document.getElementById("change_2").style.backgroundColor = "#91c1e7";
